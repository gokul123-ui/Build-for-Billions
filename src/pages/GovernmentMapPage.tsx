...
export const GovernmentMapPage: React.FC = () => {
  ...
  }, [geotagged, filterStatus, filterCategory, search, lang]);
  ...
  useEffect(() => {
    ...
    filtered.forEach(c => {
      ...
      const color = categoryColor(c.categoryId);
      const sColor = statusColor(c.status);

      // Outer ring = status, inner = category color
      const marker = L.circleMarker([g.latitude, g.longitude], {
        radius: selectedId === c.id ? 14 : 10,
        fillColor: color,
        color: selectedId === c.id ? '#fbbf24' : sColor,
        weight: selectedId === c.id ? 4 : 3,
        fillOpacity: 0.95,
      });
      marker.addTo(layer);
      const popupHtml = `
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl text-white">
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <h1 className="text-2xl sm:text-3xl font-black">Government Grievance Map — Single Map, All Complaints</h1>
            <p className="text-sm text-slate-300 max-w-2xl leading-relaxed">
              Every geotagged grievance appears pinned together. Officers see ward-level clusters, prioritize by density, and dispatch field teams with GPS accuracy — no street-name hunting.
            </p>
      ...
      {/* Filters */}
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
        ...
        <div className="lg:col-span-8 bg-slate-900 border border-slate-800 rounded-3xl p-3 sm:p-4 shadow-xl">
          ...
          <div className="flex flex-wrap gap-2 mt-3 text-[11px] font-bold px-1">
            <span className="flex items-center space-x-1.5">
              <span className="w-3 h-3 rounded-full bg-slate-700 border-2 border-white" /> <span className="text-slate-400">Roads</span>
            </span>
            <span className="flex items-center space-x-1.5">
              <span className="w-3 h-3 rounded-full bg-sky-500 border-2 border-white" /> <span className="text-slate-400">Water</span>
            </span>
            <span className="flex items-center space-x-1.5">
              <span className="w-3 h-3 rounded-full bg-amber-500 border-2 border-white" /> <span className="text-slate-400">Electricity</span>
            </span>
            <span className="flex items-center space-x-1.5">
              <span className="w-3 h-3 rounded-full bg-emerald-500 border-2 border-white" /> <span className="text-slate-400">Garbage</span>
            </span>
            <span className="ml-auto text-slate-500">Outer ring = status color • Inner = category</span>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Pinned', value: filtered.length, sub: `${geotagged.length} geotagged overall` },
          { label: 'High Priority', value: filtered.filter(c => c.priority === 'High').length, sub: 'Urgent field dispatch' },
          { label: 'In Progress', value: filtered.filter(c => c.status === 'In Progress' || c.status === 'Assigned').length, sub: 'Active field work' },
          { label: 'Resolved', value: filtered.filter(c => c.status === 'Resolved').length, sub: 'Completed' },
        ].map(card => (
